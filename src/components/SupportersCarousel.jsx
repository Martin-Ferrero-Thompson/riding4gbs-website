...
export function SupportersCarousel({ supporters, title, footerText, footerHtml }) {
  ...
  return (
    <div className="bg-gray-100 rounded-xl p-6 md:p-12 text-center">
      ...
      >
        ...
        >
          {extendedSupporters.map((supporter, index) => (
            <li key={index} className="mx-3 md:mx-4 flex-shrink-0 bg-white p-4 rounded-lg shadow-sm w-48 md:w-56">
      ...
      {footerHtml ? (
        <p className="mt-6 md:mt-8 max-w-2xl mx-auto text-base md:text-lg text-gray-600" dangerouslySetInnerHTML={{ __html: footerHtml }} />
      ) : footerText ? (
        <p className="mt-6 md:mt-8 max-w-2xl mx-auto text-base md:text-lg text-gray-600">{footerText}</p>
      ) : null}
    </div>
  );
}