...
export function Carousel({ slides, slidesMap, i18n }) {
  const [currentSlides, setCurrentSlides] = useState(slides || (slidesMap ? slidesMap.es : []));
  const [currentIndex, setCurrentIndex] = useState(0);
  const contentRef = useRef(null);
  const totalSlides = currentSlides ? currentSlides.length : 0;
  ...
  useEffect(() => {
    const updateSlides = async () => {
      if (slidesMap) {
        ...
        if (lang === 'eu' && slidesMap.eu && slidesMap.eu.length > 0) {
          setCurrentSlides(slidesMap.eu);
          setCurrentIndex(0);
        } else if (slidesMap.es && slidesMap.es.length > 0) {
          setCurrentSlides(slidesMap.es);
          setCurrentIndex(0);
        }
    ...
    return () => {
      document.removeEventListener('language-change', handleLangChange);
    };
  }, [slides, slidesMap]);
